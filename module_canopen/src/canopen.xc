  ...
  while(1)
  {
    ...
    select
    {
      ...
      switch(frame.id)
      {
        ...
        else
        {
          switch(frame.data[0])
          {
            ...
            lss_inquire_product_code(c_rx_tx); //send product code
            break;
        ...
        if(canopen_state == OPERATIONAL)
        {
          ...
          else
          {
            ...
            {
              unsigned rtr_check;
              rtr_check = ((pdo_find_cob_id(sync_timer[pdo_number].comm_parameter) >> 30) &0x3);
          ...
          while(pdo_number != CANOPEN_NUMBER_OF_TPDOS_SUPPORTED)
          {
            if(pdo_event[pdo_number].event_type != 0)
            {
              ...
              if(pdo_event[pdo_number].counter == pdo_event[pdo_number].event_type)
              {
                ...
                {
                  pdo_transmit_data(TPDO_0_COMMUNICATION_PARAMETER + pdo_number, TPDO_0_MAPPING_PARAMETER + pdo_number, c_rx_tx);
      ...
      if(canopen_state == OPERATIONAL)
      {
        ...
        pdo_receive_application_data(app_tpdo_number, app_length, app_data, tpdo_inhibit_time_values, c_rx_tx);
